...
let five1 : lseq uint8 1 = createL five1_list
...
(* Compute ciphertext length from plaintex length *)
...
let hmac mk plain =
  ...
  Spec.Agile.HMAC.hmac Spec.Hash.Definitions.SHA2_256 mk plain


val hkdf1: key:lbytes_sub 160 -> salt:lbytes 32 -> info:lbytes_sub 32  -> Tot (lbytes 32)
let hkdf1 key salt info =
  let extracted = Spec.Agile.HKDF.extract Spec.Hash.Definitions.SHA2_256 salt key in
  Spec.Agile.HKDF.expand Spec.Hash.Definitions.SHA2_256 extracted info 32

val hkdf2: key:lbytes 32 -> salt:lbytes 32 -> info:lbytes_sub 32 -> lbytes 64
let hkdf2 key salt info =
  let extracted = Spec.Agile.HKDF.extract Spec.Hash.Definitions.SHA2_256 salt key in
  Spec.Agile.HKDF.expand Spec.Hash.Definitions.SHA2_256 extracted info 64

val hkdf3: key:lbytes 32 -> salt:lbytes 32 -> info:lbytes_sub 32 -> lbytes 96
let hkdf3 key salt info =
  let extracted = Spec.Agile.HKDF.extract Spec.Hash.Definitions.SHA2_256 salt key in
  Spec.Agile.HKDF.expand Spec.Hash.Definitions.SHA2_256 extracted info 96
