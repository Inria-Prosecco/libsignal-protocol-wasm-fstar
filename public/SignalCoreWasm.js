...
(function() {
    ...
    var my_imports = {
      "Impl_Signal_Crypto": function(mem) {
        return {
          "random_bytes": function (len, output) {
            let bytes = new ArrayBuffer(len);
            crypto.getRandomValues(new Uint8Array(bytes));
            let memory = new Uint8Array(mem.buffer);
            memory.set(new Uint8Array(bytes), output);
          }
        };
      }
    }
    ...
    const checkIfInitialized = function() {
        if (isInitialized === false) {
          return Promise.all(my_modules.map(m => {
              ...
              return link(my_imports, bufs.map((b, i) => ({ buf: b, name: my_modules[i] })))
            }).then(scope => {
              SignalStar = scope;
              isInitialized = true;
              console.log("DEBUG: SignalStar:", SignalStar);
            })
        } else {
    ...
    */
    ...
    const FStarEncrypt = function (maxOutputSize) {
      return {
        ...
        "return": [
          { "type": "varbuf", "size": maxOutputSize },
        ]
      };
    }
    ...
    const malloc_array = function(array) {
        let pointer = reserve(SignalStar.Kremlin.mem, array.length);
        let memory = new Uint8Array(SignalStar.Kremlin.mem.buffer);
    ...
    const read_memory = function(ptr, len) {
        ...
        (new Uint8Array(result).set(new Uint8Array(SignalStar.Kremlin.mem.buffer)
    ...
    const callWithProto = function (proto, args) {
      ...
      let args_pointers = args.map((arg,i) => {
        ...
        if (protoArg.type === "varbuf") {
          ...
          let pointer = malloc_array(argByteBuffer);
          return [
            { "value": argByteBuffer.byteLength, "index": null },
            { "value": pointer, "index": i }
          ];
        }
